...
def computeG(X,gamma):
    ...
    for i in range(len(X)):
        for j in range(len(X)):
            G[i,j]=minmaxkernel(X[i],X[j],gamma)
    return G

def get_fq(x,q,N,M,A,points,gamma):
    value1=0
    for n in range(N):
        value1+= A[n,q]*minmaxkernel(data[n],x,gamma)
    value2=0
    for m in range(N,M+N):
        value2+= A[m,q]*minmaxkernel(points[m-N],x,gamma)
        ...
        for lmdg in lmdg_v:
            for lmdb in b_v:
                ...
                for i in range(2):
                    ...
                    for q in range(Q):
                        for n in range(N):
                            obj+=expectile(y[n]-((Gn@A)[n,q]+b[q]),tau[q])
...
for q in range(Q-1,0,-1):
    ...
    plt.plot(seq,fq,label='$\\alpha={}$'.format(tau[q]))
