...
def sample_examples(docs,max_window_size,n_windows):
    ...
    all_negs = list(np.random.choice(token_ints, 
                                     p=neg_distr
                                     , size=n_negs*len(token_ints))) 
...
def compute_loss(prodpos,prodnegs):
    '''prodpos and prodnegs are numpy vectors containing the dot products of
    the context word vectors with the target word vector'''
    ### fill the gaps ###
...
def compute_gradients(pos,negs,target,prodpos,prodnegs):
    ...
    ### fill the gaps ### 
    term_pos = (-Wc[pos,].T *( np.exp(-prodpos) * factors_pos )).T
    term_negs = (Wc[negs,].T*( np.exp(prodnegs) * factors_negs )).T
    partial_target = np.sum(term_pos, axis = 0) + np.sum(term_negs, axis = 0)
    p=np.reshape((np.exp(-prodpos) *factors_pos),(len(pos),1))
    w= np.reshape(Wt[target,],(d,1))
    n=np.reshape((np.exp(prodnegs) *factors_negs),(len(negs),1))
    pos_c = p@w.T
    neg_c = n@w.T
    partials_pos = pos_c
    partials_negs = neg_c 
...
path_read = "D://for_moodle//data//"
path_write = "D://for_moodle//data//"
...
if train:
    ...
    Wt = np.random.normal(size=(len(vocab)+1,d)) * np.sqrt(1/len(vocab))# + 1 is for the OOV token
    Wc = np.random.normal(size=(len(vocab)+1,d)) * np.sqrt(1/len(vocab))
    ...
    Wt = np.load('D://for_moodle//data//input_vecs.npy')
...
if not train:
    ...
    fig, ax = plt.subplots()
    ax.scatter(my_tsne_fit[:,0],my_tsne_fit[:,1]) ### fill the gap ###
